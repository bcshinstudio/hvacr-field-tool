...
export async function loadKnowledge(base="../data/knowledge"){
 const [manifest,concepts,rules,relationships,checks,discriminators,sources,application]=await Promise.all([
  ...
  loadJson(`${base}/sources/sources.json`),loadJson(`${base}/applications/walk_in_cooler.json`)]);
 return {manifest,concepts,rules,relationships,checks,discriminators,sources,application};
}
